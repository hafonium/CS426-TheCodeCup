...
class CartItemFoodOptionTypeModel(Base):
    ...
    cart_item_id: Mapped[int] = mapped_column(ForeignKey("CART_ITEMS.id", ondelete="CASCADE"), primary_key=True, index=True)
    food_option_type_id: Mapped[int] = mapped_column(ForeignKey("FOOD_OPTION_TYPES.id", ondelete="CASCADE"), primary_key=True, index=True)
